...
// For typescript users: create the root reducer independently to obtain the RootState type
const rootReducer = combineReducers({
  user: userReducer
})

// Rather than export the store directly, export a function that returns the store.
// This allows us to use a separate store instance for each test
...
To test the component, we `render` it into the DOM, and assert that the app responds to interactions in the way we expect the user to use the app. We can use the `wrapper` option in the `render` function and export our own `renderWithProviders` function as based on React Testing Library's [setup docs](https://testing-library.com/docs/react-testing-library/setup).

Our `renderWithProviders` function can look like this:
...
// Import your store setup function
...
/**
 * Use in place of `render` to wrap the rendered element with our app providers,
 * with a unique store instance per call.
 */
...
And our test can use our exported `renderWithProviders` function to test the criteria of our integration test:
...
// file: test/test-utils.tsx noEmit
// test-utils.tsx
import React from 'react'
import { render, RenderOptions } from '@testing-library/react'
import { configureStore, PreloadedState } from '@reduxjs/toolkit'
...
// We're using our own custom render function which extends RTL's render.
...
test('fetches & receives a user after clicking the fetch user button', async () => {
  ...
  userEvent.click(screen.getByRole('button', { name: /Fetch user/i }))
  expect(screen.getByText(/no user/i)).toBeInTheDocument()
...
export const create = () => {
  const store = {
    getState: jest.fn(() => ({ user: { name: 'No user', status: 'idle' } })),
    dispatch: jest.fn()
  }
  const next = jest.fn()
